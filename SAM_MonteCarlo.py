# -*- coding: utf-8 -*-
"""
Created on Thu Feb 14 10:35:47 2019
...
edge_list = sam_full.get_edgelist()
sim_graph_0 = Graph()
full_EL = sam_full.get_edgelist() # Full list of possible edges to make or break
...
for i in range(0,nsteps):  
    ...
    # Set initial energy:
    ener_0 = EnerState(sim_graph_0)
    
    # Calculate graph after move:
    # Does selected edge exist in simulated graph?:
    #print any(full_EL[edge_sel] for j in sim_EL_0)
    ...
    #print sim_EL_1, full_EL[edge_sel]
        
    ener_1 = EnerState(sim_graph_1)
    
    # Calculate the probability of making the move:
    prob=np.exp(-(ener_1-ener_0)/kT)/(1+np.exp(-(ener_1-ener_0)/kT))
    #print ener_1,ener_0,any([full_EL[edge_sel]==sim_EL_0[j] for j in range(0,np.shape(sim_EL_0)[0])])
    
    criterion = random.uniform(0, 1)
    if criterion < prob:
        sim_graph_0 = Graph(sim_EL_1)
        #print 'Step:',i,'Probability is',prob,'Move: YES',np.shape(sim_EL_1)[0]
        #print 'Step:',i,criterion < prob,any([full_EL[edge_sel]==sim_EL_0[j] for j in range(0,np.shape(sim_EL_0)[0])]),np.shape(sim_EL_0),np.shape(sim_EL_1)
    #elif criterion >0:
    #    x=1
        #print 'Step:',i,'Probability is',prob,'Move: NO', np.shape(sim_EL_1)[0]
        #print 'Step:',i,criterion < prob,any([full_EL[edge_sel]==sim_EL_0[j] for j in range(0,np.shape(sim_EL_0)[0])]),np.shape(sim_EL_0),np.shape(sim_EL_1)
