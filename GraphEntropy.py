# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 13:39:02 2019

Script to calculate number of theoretical trimers and tetramers on a
...
n_triangles, n_tetramers = np.zeros(100),np.zeros(100)
...
for n_lig in range(4,51):
    strand_id = x*0
    k = 0
    for j in range(0,2*int(np.ceil(np.sqrt(n_lig)))): #np.shape(strand_id)[0]):
        for i in range(0,2*int(np.ceil(np.sqrt(n_lig)))): #np.shape(strand_id)[1]):
            if k < n_lig:
                ...
                if (index1 == 1)&(index2 == 0):
                    ...
                    k+=1
#    # Plot:
#    ax.scatter(x,y,c=strand_id,s=5)
#    k=0
...
#    plt.show()
    ...
    n_triangles[n_lig] = np.shape(sam_full.maximal_cliques(min=3,max=3))[0]
    n_tetramers[n_lig] = FindTetrahedraFast(sam_full)
    #print n_triangles, n_tetramers
fig, ax = plt.subplots(figsize=(5,4))
ax.plot(-2.48*np.log(n_triangles))
ax.plot(-2.48*np.log(n_tetramers))
